 ...
 * Combined and modified for PDFPals
 */
...
(function (window) {
    ...
    function md5(data) {
        ...
        const result = new Uint8Array(16), view = new DataView(result.buffer);
        view.setUint32(0, a0, true); view.setUint32(4, b0, true); view.setUint32(8, c0, true); view.setUint32(12, d0, true);
        return result;
    }
    ...
    function computeEncryptionKey(userPassword, ownerKey, permissions, fileId) {
        ...
        let offset = 0; hashInput.set(paddedPwd, offset); offset += paddedPwd.length;
        hashInput.set(ownerKey, offset); offset += ownerKey.length;
        hashInput[offset++] = permissions & 0xFF; hashInput[offset++] = (permissions >> 8) & 0xFF;
        hashInput[offset++] = (permissions >> 16) & 0xFF; hashInput[offset++] = (permissions >> 24) & 0xFF;
    ...
    function computeUserKey(encryptionKey, fileId) {
        ...
        const finalResult = new Uint8Array(32); finalResult.set(result); finalResult.set(new Uint8Array(16), 16);
        return finalResult;
    }

    function encryptObject(data, objectNum, generationNum, encryptionKey) {
        const keyInput = new Uint8Array(encryptionKey.length + 5); keyInput.set(encryptionKey);
        keyInput[encryptionKey.length] = objectNum & 0xFF; keyInput[encryptionKey.length + 1] = (objectNum >> 8) & 0xFF; keyInput[encryptionKey.length + 2] = (objectNum >> 16) & 0xFF;
        keyInput[encryptionKey.length + 3] = generationNum & 0xFF; keyInput[encryptionKey.length + 4] = (generationNum >> 8) & 0xFF;
    ...
    function encryptStringsInObject(obj, objectNum, generationNum, encryptionKey) {
        if (!obj) return;
        const { PDFString, PDFHexString, PDFDict, PDFArray } = window.PDFLib;
        if (obj instanceof PDFString) {
            ...
            for (const element of array) { encryptStringsInObject(element, objectNum, generationNum, encryptionKey); }
        }
    }

    async function encryptPDF(pdfBytes, userPassword, ownerPassword = null) {
        const { PDFDocument, PDFName, PDFHexString, PDFNumber, PDFDict, PDFRawStream } = window.PDFLib;
        ...
        let idArrayValue = idArray;
        if (idArray instanceof window.PDFLib.PDFArray) idArrayValue = idArray.asArray();

        let fileId;
        if (idArray && Array.isArray(idArrayValue) && idArrayValue.length > 0) {
            fileId = hexToBytes(idArrayValue[0].toString().replace(/^<|>$/g, ''));
        } else {
            fileId = crypto.getRandomValues(new Uint8Array(16));
            const idHex1 = PDFHexString.of(bytesToHex(fileId)), idHex2 = PDFHexString.of(bytesToHex(fileId));
            trailer.ID = [idHex1, idHex2];
        }
        const permissions = -4; // 0xFFFFFFFC (all allowed)
        ...
        for (const [ref, obj] of indirectObjects) {
            const objectNum = ref.objectNumber, generationNum = ref.generationNumber || 0;
            ...
            if (obj instanceof PDFRawStream && obj.dict) {
                ...
                if (type && ['/XRef', '/Sig'].includes(type.toString())) continue;
            }
            if (obj instanceof PDFRawStream) {
                obj.contents = encryptObject(obj.contents, objectNum, generationNum, encryptionKey);
                if (obj.dict) encryptStringsInObject(obj.dict, objectNum, generationNum, encryptionKey);
            } else {
                encryptStringsInObject(obj, objectNum, generationNum, encryptionKey);
            }
        }
        const encryptDict = context.obj({ Filter: PDFName.of('Standard'), V: PDFNumber.of(2), R: PDFNumber.of(3), Length: PDFNumber.of(128), P: PDFNumber.of(permissions), O: PDFHexString.of(bytesToHex(ownerKey)), U: PDFHexString.of(bytesToHex(userKey)) });
    ...
    async function decryptPDF(pdfBytes, password) {
        const { PDFDocument, PDFName, PDFHexString } = window.PDFLib;
        // Load ignoring encryption to get to the data
        const pdfDoc = await PDFDocument.load(pdfBytes, { ignoreEncryption: true, updateMetadata: false });
        const context = pdfDoc.context;
        const trailer = context.trailerInfo;
        ...
        if (filter && filter.asString() !== '/Standard') {
            throw new Error("Only Standard Security Handler is supported.");
        }

        const V = encryptDict.get(PDFName.of('V')).numberValue;
        const R = encryptDict.get(PDFName.of('R')).numberValue;
        if (V !== 2 || R !== 3) {
            throw new Error(`Unsupported encryption Version ${V} Revision ${R}. Only V2 R3 (128-bit RC4) is supported.`);
        }
        ...
        const fileId = hexToBytes(idArrayValue[0].toString().replace(/^<|>$/g, ''));

        // Try user password first
        const derivedKey = computeEncryptionKey(password, O, P, fileId);
        const expectedU = computeUserKey(derivedKey, fileId);

        // Verify U (first 16 bytes are the hash, rest is padding/flags in R3)
        let matches = true;
        for (let i = 0; i < 16; i++) {
            if (expectedU[i] !== U[i]) {
                matches = false;
        ...
        if (!matches) {
            ...
            // In R3, owner password is more complex to verify, but we can try to derive 
            // the user password hash from it. For now, we'll keep it simple: 
            // the user usually provides the user password.
        ...
        for (const [ref, obj] of indirectObjects) {
            ...
            if (obj instanceof window.PDFLib.PDFRawStream) {
                obj.contents = encryptObject(obj.contents, objectNum, generationNum, derivedKey);
                if (obj.dict) encryptStringsInObject(obj.dict, objectNum, generationNum, derivedKey);
            } else {
                encryptStringsInObject(obj, objectNum, generationNum, derivedKey);
            }
        }
    ...
    // Export to window
    window.PDFEncryptLite = { encryptPDF, decryptPDF };
